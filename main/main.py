#!/usr/bin/python
# -*- coding: utf-8 -*-
import tkinter as tk
import threading
import queue
import time
import re
import winsound

# 复选框状态

from app.Filter import Filter
from app.List import List
from app.Serial import Serial
from app.WarnValue import WarnValue

from app.data_record import DataRecord

if __name__ == '__main__':

    upCheckStatus = 0
    downCheckStatus = 1
    checkCount = 0
    autoMode = False


    def CheckCallBack():
        global checkCount
        global upCheckStatus
        global downCheckStatus
        checkCount = checkCount + 1
        if (checkCount % 2):
            upCheckStatus = 1
            downCheckStatus = 0
        else:
            upCheckStatus = 0
            downCheckStatus = 1
        # print("ok")
        return


    warnValue = WarnValue(0)

    # 串口数据

    rt = Serial()
    SerStatus = 0
    SerCount = 0
    SerCom = '11'
    dataQueue = queue.Queue(1000)
    listArr = List()
    file_handle = open('.1.txt', mode='w')
    myFilter = Filter()
    top = tk.Tk()
    top.geometry('600x600')
    top.resizable(1, 1)
    # line
    cv = tk.Canvas(top, bg='white', width=480, height=286)
    cv.place(x=5, y=105, anchor=tk.NW)
    cv.create_text(10, 10, text=str(warnValue.getUpValue()))
    cv.create_text(10, 270, text=str(warnValue.getDownValue()))

    CValue = 1
    RStatus = 0

    dt = DataRecord('../data/SF6/BD-zd2')
    dtFlag = False
    Var = 0.25


    def RecordCallback():
        global dt


    def ReceiveTask():
        global rt
        global dataQueue
        while t4_flag:
            time.sleep(0.01)
            n = rt.l_serial.inWaiting()  # 获取接收到的数据长度
            if n:
                data = ((rt.l_serial.read_all().decode('utf-8')))
                # print('get data from serial port:' + data)
                if (re.search('[8][0]:', data) != None):
                    res = str(data).split(":")
                    l = len(res[1])
                    num = res[1][0:l - 2]
                    # print(str(num))
                    if (re.search('\-?[0-9]*', num) != None):
                        try:
                            dataQueue.put_nowait(float(num))
                        except ValueError:
                            pass
                    # print(str(float(num)))


    t4 = threading.Thread(target=ReceiveTask)
    t4_flag = True


    def openSer():
        global rt
        try:
            if rt.start():
                rt.waiting()
            else:
                pass
        except Exception as se:
            pass


    def closeSer():
        global rt
        try:
            rt.stop()
        except Exception as se:
            pass


    def connectCallback():
        global SerCount
        global comB
        global dataPath
        global dt
        global rt
        global t4
        global t4_flag
        global top
        SerCount = SerCount + 1
        if (SerCount % 2):
            dt.set_path(dataPath.get())
            com = comB.get()
            OpenColor.set("已打开")
            rt.setPort(com)
            openSer()
            t4_flag = True
            t4 = threading.Thread(target=ReceiveTask)
            t4.start()
        else:
            t4_flag = False
            OpenColor.set("已断开")
            closeSer()
            # exit()
            #         # quit()
            #         # top.destroy()
            #         # win.protocol("WM_DELETE_WINDOW", lambda: sys.exit(0));


    def Upavlue10CallBack():
        global upCheckStatus, downCheckStatus
        if (upCheckStatus == 1 and downCheckStatus == 0):
            warnValue.AddUp10()
            UpVar.set("上限:" + str(warnValue.getUpValue()))
        elif (upCheckStatus == 0 and downCheckStatus == 1):
            warnValue.AddDown10()
            DownVar.set("下限:" + str(warnValue.getDownValue()))


    def Upavlue1CallBack():
        global upCheckStatus, downCheckStatus
        if (upCheckStatus == 1 and downCheckStatus == 0):
            warnValue.AddUp1()
            UpVar.set("上限:" + str(warnValue.getUpValue()))
        elif (upCheckStatus == 0 and downCheckStatus == 1):
            warnValue.AddDown1()
            DownVar.set("下限:" + str(warnValue.getDownValue()))


    def Downavlue10CallBack():
        global upCheckStatus, downCheckStatus
        if (upCheckStatus == 1 and downCheckStatus == 0):
            warnValue.SubUp10()
            UpVar.set("上限:" + str(warnValue.getUpValue()))
        elif (upCheckStatus == 0 and downCheckStatus == 1):
            warnValue.SubDown10()
            DownVar.set("下限:" + str(warnValue.getDownValue()))


    def Downavlue1CallBack():
        global upCheckStatus, downCheckStatus
        if (upCheckStatus == 1 and downCheckStatus == 0):
            warnValue.SubUp1()
            UpVar.set("上限:" + str(warnValue.getUpValue()))
        elif (upCheckStatus == 0 and downCheckStatus == 1):
            warnValue.SubDown1()
            DownVar.set("下限:" + str(warnValue.getDownValue()))


    UpVar = tk.StringVar()
    UpVar.set("上限:" + str(warnValue.getUpValue()))
    DownVar = tk.IntVar()
    DownVar.set("下限:" + str(warnValue.getDownValue()))
    OpenColor = tk.StringVar()
    OpenColor.set("连接")
    dtInfo = tk.StringVar()
    dtInfo.set("[INFO]" + '')
    dtPrint = tk.StringVar()
    dtPrint.set(str(dt.data_list))
    dtPath = tk.StringVar()
    dtPath.set(str(dt.path))
    dtNo = tk.StringVar()
    dtNo.set(str(dt.No))
    dtVar = tk.StringVar()
    dtVar.set(str(Var))
    dtVarReal = tk.StringVar()
    dtVarReal.set(str(dt.var))

    # 文件
    FileLabel = tk.StringVar()
    FileLabel.set("日志")
    logCount = 1


    def logFile():
        global file_handle
        global logCount
        if (logCount % 2):
            localtime = time.asctime(time.localtime(time.time()))
            filename = "日志" + str(localtime) + '.txt'
            filename = filename.replace(' ', '-')
            filename = filename.replace(':', '-')
            file_handle = open(filename, mode='w')
            FileLabel.set("正常")
        else:
            file_handle.close()
            FileLabel.set("完成")
        logCount = logCount + 1


    def dataRecord():
        global dt
        global dtInfo
        global dtNo
        global dtPath
        dt.set_path(dtPath.get())
        dt.set_No(int(dtNo.get()))
        dtInfo.set(dt.data_record())
        dtPath.set(dt.path)
        dtNo.set(str(dt.No))


    def dataClear():
        global dt
        global dtPrint
        dt.data_clear()
        dtPrint.set(str(dt.data_list))


    def autoRecord(status):
        global dt
        global dtInfo
        global dtNo
        global autoMode
        autoMode = status
        if autoMode:
            dtInfo.set('[INFO]' + 'AUTO recording')
        else:
            dtInfo.set('[INFO]' + 'AUTO mode closed')
            # dt.data_clear()


    def autoRecordCallback():
        global dt
        dataRecord()
        winsound.Beep(800, 1000)
        dt.data_clear()


    im = b''
    # log 图片
    im = tk.PhotoImage(data=im)
    logo = tk.Label(top, anchor=tk.NW, image=im)
    logo.place(x=0, y=0, anchor=tk.NW)
    # down1
    down1B = tk.Button(top, width=6, height=1, text="-0.1", bg="white", command=Downavlue1CallBack)
    down1B.place(x=525, y=65, anchor=tk.NW)
    # up1
    up1B = tk.Button(top, width=6, height=1, text="+0.1", bg="white", command=Upavlue1CallBack)
    up1B.place(x=472, y=65, anchor=tk.NW)
    # down10
    down10B = tk.Button(top, width=6, height=1, text="-1", bg="white", command=Downavlue10CallBack)
    down10B.place(x=400, y=65, anchor=tk.NW)
    # up10
    up10B = tk.Button(top, width=6, height=1, text="+1", bg="white", command=Upavlue10CallBack)
    up10B.place(x=347, y=65, anchor=tk.NW)

    # com
    comText = tk.Label(top, width=5, height=1, text="COM", )
    comText.place(x=488, y=110, anchor=tk.NW)
    comB = tk.Entry(top, bd=5, width=5)
    comB.place(x=525, y=110, anchor=tk.NW)
    # open
    openB = tk.Button(top, width=8, height=1, text="连接", textvariable=OpenColor, bg="white", command=connectCallback)
    openB.place(x=515, y=165, anchor=tk.NW)
    # close
    closeB = tk.Button(top, width=8, height=1, text="日志", textvariable=FileLabel, bg="white", command=logFile)
    closeB.place(x=515, y=220, anchor=tk.NW)
    # save
    saveB = tk.Button(top, width=8, height=1, text="保存", bg="white", command=logFile)
    saveB.place(x=515, y=275, anchor=tk.NW)
    # clear
    clearB = tk.Button(top, width=8, height=3, text="校准", bg="white")
    clearB.place(x=515, y=330, anchor=tk.NW)

    # data_list_text
    dataText = tk.Label(top, width=82, height=6, text='', textvariable=dtPrint, bg="grey", wraplength=550)
    dataText.place(x=10, y=410, anchor=tk.NW)

    # path_text
    dataPathText = tk.Label(top, width=5, height=1, text="Path", )
    dataPathText.place(x=10, y=530, anchor=tk.NW)
    # data_path
    dataPath = tk.Entry(top, bd=5, width=40, textvariable=dtPath)
    dataPath.place(x=50, y=530, anchor=tk.NW)

    # no_text
    dataNoText = tk.Label(top, width=3, height=1, text="No", )
    dataNoText.place(x=350, y=530, anchor=tk.NW)
    # data_no
    dataNo = tk.Entry(top, bd=5, width=10, textvariable=dtNo)
    dataNo.place(x=380, y=530, anchor=tk.NW)
    # no_text
    dataVarText = tk.Label(top, width=3, height=1, text="Var", )
    dataVarText.place(x=470, y=530, anchor=tk.NW)
    # data_no
    dataVar = tk.Entry(top, bd=5, width=10, textvariable=dtVar)
    dataVar.place(x=500, y=530, anchor=tk.NW)

    # save_data
    saveD = tk.Button(top, width=10, height=1, text="保存数据", bg="white", command=dataRecord)
    saveD.place(x=10, y=565, anchor=tk.NW)
    # clear_data
    clearD = tk.Button(top, width=10, height=1, text="清除数据", bg="white", command=dataClear)
    clearD.place(x=110, y=565, anchor=tk.NW)
    # keep_auto
    openB = tk.Button(top, width=8, height=1, text="自动", bg="white", command=lambda: autoRecord(not autoMode))
    openB.place(x=210, y=565, anchor=tk.NW)

    dataInfo = tk.Label(top, width=30, height=1, text='[INFO]', textvariable=dtInfo, bg="grey")
    dataInfo.place(x=300, y=565, anchor=tk.NW)

    dataInfo = tk.Label(top, width=6, height=1, text='', textvariable=dtVarReal, bg="grey")
    dataInfo.place(x=530, y=565, anchor=tk.NW)

    # upval
    Upcheck = tk.Checkbutton(top, textvariable=UpVar, font=6, command=CheckCallBack, variable=1, onvalue=1, offvalue=0)
    Upcheck.place(x=10, y=65, anchor=tk.NW)
    # downval
    Downcheck = tk.Checkbutton(top, textvariable=DownVar, font=6, command=CheckCallBack, variable=1, onvalue=0,
                               offvalue=1)
    Downcheck.place(x=180, y=65, anchor=tk.NW)


    def DrawTask():
        global cv
        global dt
        global dtPrint
        global dtVar
        global autoMode
        global avrage
        global warnValue
        global dataQueue
        while True:
            num = dataQueue.get(timeout=10000)
            print("get " + str(num))
            dt.add(num)
            if dt.check_var(float(dtVar.get())) and autoMode:
                s = threading.Timer(10, autoRecordCallback)
                s.start()
                # dt.data_record()
                autoRecord(False)
                winsound.Beep(400, 500)
            dtVarReal.set("{:.2f}".format(dt.var))
            dtPrint.set(str(dt.data_list))
            # cnum = float(num)
            cnum = myFilter.filter(float(num))
            listArr.add(cnum)
            list = listArr.get()
            i = 0
            j = 0
            mc = 0
            cv.delete(tk.ALL)
            # cv.create_text(460, 10, text=str(int(CValue)))
            # cv.create_text(10, 10, text=str(warnValue.getUpValue()))
            # cv.create_text(10, 270, text=str(warnValue.getDownValue()))
            cv.create_line(0, 143, 480, 143, fill='black', width=2)
            # print("log  "+str(avrage) +" "+ str(warnValue.getUpValue())+" "+str(warnValue.getDownValue()))
            if (warnValue.getUpValue() - warnValue.getDownValue() != 0):
                list[47] = 143 + (list[47]) / (warnValue.getUpValue() - warnValue.getDownValue()) * 1430
            else:
                list[47] = 143
            while (i < 470):
                cv.create_line(i, list[j], i + 10, list[j + 1], fill='blue', width=2)  # xyxy
                i = i + 10
                j = j + 1
            time.sleep(0.01)


    dra = threading.Thread(target=DrawTask)
    dra.start()

    # 进入消息循环
    top.mainloop()
